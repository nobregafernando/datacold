...
class PaginaSensor {
  ...
  async _carregarDados() {
    ...
    // sem chave: mostra o banner + limpa todos os painéis
    if (!this.api.chave) {
      document.querySelector("[data-aviso-chave]").hidden = false;
      this._renderizarConectividade(null, "sem-chave");
      this._estadoSemDados({
        titulo: "Sem chave de API",
        msg: "Configure sua X-API-Key no banner amarelo acima para começar a receber leituras.",
      });
      return;
    }
    document.querySelector("[data-aviso-chave]").hidden = true;
    ...
    try {
      ...
      this.dados = await this.api.buscarDados(this.sensorId, { inicio, fim, limite: 1000 });
  ...
  _avaliarStatus(dados, ultimo) {
    if (this.sensor.tipo === "energia") {
      ...
      if (fpComp < 0.85) return { sev: "warn", emoji: "⚠️", titulo: "Fator de potência baixo", sub: `FP composto = ${fpComp.toFixed(2)} (limite ANEEL 0,92).`, valor: `${pot.toFixed(1)} kW` };
    ...
    if (this.sensor.tipo === "temperatura") {
      ...
      if (Math.abs(t) > 100) return { sev: "crit", emoji: "🛑", titulo: "Leitura impossível", sub: "Sensor com defeito — recalibrar ou trocar.", valor: `${t.toFixed(1)}°C` };
      if (faixa) {
  ...
  _renderizarVelocimetrosEnergia(visual, ultimo) {
    ...
    const max = 250; // escala fixa de referência (motor industrial típico)
    ["a","b","c"].forEach(f => {
      const card = visual.querySelector(`[data-gauge="${f}"]`);
      const v = ultimo[`corrente_fase_${f}`] || 0;
      const pct = Math.max(0, Math.min(1, Math.abs(v) / max));
      const sev = pct < 0.6 ? "ok" : pct < 0.85 ? "warn" : "crit";
