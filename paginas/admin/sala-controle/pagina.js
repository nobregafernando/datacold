...
(async () => {
  ...
  // ===================================================================
  // Todas as falhas duram 60s e o backend libera sozinho quando `fim <= now()`
  // (ver supabase/simulador_db.sql `sim_aplicar_incidentes`). Mantemos as
  // magnitudes originais — só o tempo encurta pra demos rápidas no pitch.
  const PRESETS = {
    energia: [
      { acao:"pico_corrente",  titulo:"Pico de corrente",        sub:"spike 2.5× · 1 min",  tipo:"spike",            magnitude:2.5,  duracaoS:60,
        severidade:"critica",  descricao:"Pico de corrente súbito por 1 min (motor travando ou partida pesada) — depois volta ao normal" },
      { acao:"queda_fp",       titulo:"Queda de FP",             sub:"drift −0.4 · 1 min",  tipo:"drift",            magnitude:-0.4, duracaoS:60,
        severidade:"alta",     descricao:"Fator de potência cai 0,4 por 1 min — depois volta ao normal" },
      { acao:"sobrecarga",     titulo:"Sobrecarga térmica",      sub:"drift +30A · 1 min",  tipo:"drift",            magnitude:30,   duracaoS:60,
        severidade:"alta",     descricao:"Corrente sobe +30 A por 1 min (motor sobrecarregado) — depois volta ao normal" },
      { acao:"desconectado",   titulo:"Desconectar da rede",     sub:"gap · 1 min e meio",  tipo:"gap",              magnitude:0,    duracaoS:90,
        severidade:"media",    descricao:"Sensor perde conectividade por 1 min e meio — depois reconecta sozinho" },
      { acao:"offline",        titulo:"Equipamento parou",       sub:"offline · 1 min",     tipo:"offline",          magnitude:0,    duracaoS:60,
        severidade:"critica",  descricao:"Equipamento desligado por 1 min — depois religa sozinho" },
      { acao:"valor_zoado",    titulo:"Leitura corrompida",      sub:"valor= −999 · 1 min", tipo:"valor_impossivel", magnitude:0, valor:-999, duracaoS:60,
        severidade:"media",    descricao:"Sensor reporta valor impossível (−999) por 1 min — depois volta ao normal" },
    ],
    temperatura: [
      { acao:"superaquecer",   titulo:"Superaquecimento progressivo", sub:"drift +15°C · 1 min", tipo:"drift",            magnitude:15,   duracaoS:60,
        severidade:"critica",  descricao:"Temperatura sobe +15°C por 1 min — depois volta ao normal" },
      { acao:"pico_calor",     titulo:"Pico súbito de calor",         sub:"drift +25°C · 1 min", tipo:"drift",            magnitude:25,   duracaoS:60,
        severidade:"critica",  descricao:"Pico instantâneo de +25°C por 1 min — depois volta ao normal" },
      { acao:"congelar",       titulo:"Congelamento extremo",         sub:"drift −15°C · 1 min", tipo:"drift",            magnitude:-15,  duracaoS:60,
        severidade:"alta",     descricao:"Temperatura cai −15°C por 1 min — depois volta ao normal" },
      { acao:"travado",        titulo:"Sensor travado",               sub:"valor= −22°C · 1 min", tipo:"valor_impossivel", magnitude:0, valor:-22, duracaoS:60,
        severidade:"alta",     descricao:"Sensor reporta sempre −22°C por 1 min — depois volta ao normal" },
      { acao:"desconectado",   titulo:"Desconectar da rede",          sub:"gap · 1 min e meio",  tipo:"gap",              magnitude:0,    duracaoS:90,
        severidade:"media",    descricao:"Sensor para de enviar dados por 1 min e meio — depois reconecta sozinho" },
      { acao:"valor_zoado",    titulo:"Leitura absurda",              sub:"valor= +85°C · 1 min", tipo:"valor_impossivel", magnitude:0, valor:85, duracaoS:60,
        severidade:"media",    descricao:"Pico de leitura corrompida em +85°C por 1 min — depois volta ao normal" },
    ],
    porta: [
      { acao:"porta_aberta",   titulo:"Porta presa aberta",           sub:"drift +100 · 1 min",  tipo:"drift",            magnitude:100,  duracaoS:60,
        severidade:"critica",  descricao:"Porta forçada aberta por 1 min — depois fecha sozinha" },
      { acao:"oscilacao",      titulo:"Porta oscilando",              sub:"spike 2× · 1 min",    tipo:"spike",            magnitude:2,    duracaoS:60,
        severidade:"alta",     descricao:"Sinal oscilando por 1 min (porta batendo) — depois volta ao normal" },
      { acao:"travado",        titulo:"Sensor travado",               sub:"valor= 0 · 1 min",    tipo:"valor_impossivel", magnitude:0, valor:0, duracaoS:60,
        severidade:"alta",     descricao:"Sensor de porta sem variação por 1 min — depois volta ao normal" },
      { acao:"desconectado",   titulo:"Desconectar da rede",          sub:"gap · 1 min e meio",  tipo:"gap",              magnitude:0,    duracaoS:90,
        severidade:"media",    descricao:"Sensor para de enviar dados por 1 min e meio — depois reconecta sozinho" },
      { acao:"offline",        titulo:"Sensor offline",               sub:"offline · 1 min",     tipo:"offline",          magnitude:0,    duracaoS:60,
        severidade:"critica",  descricao:"Sensor totalmente offline por 1 min — depois religa sozinho" },
    ],
  };
